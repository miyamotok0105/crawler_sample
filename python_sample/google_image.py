...
class GoogleImageExtractor(object):
    ...
    def __init__(self, search_key = '' ):
        ...
        self.folder_main_dir_prefix = r'./data/image_pic'
    ...
    def extract_pic_url(self):
        ...
        """
        tag_list = DOM(self.page_source)
        # print(dom)
        # a = dom.getElementsByTagName("a")
        # print(a)
        # tag_list = dom
        # tag_list = dom('a.rg_ic')
        # a = tag.by_tag("a")
        ...
        for tag in tag_list[:self.image_dl_per_search]:
            ...
            #todo:base64形式のimageを取る部分で苦戦中
            pattern = re.compile(r'data:image\/([a-zA-Z]*);base64,([^\"]+[)])')
            matchedList = pattern.findall(str(tag))
            print(matchedList)
            print("!!")

            for m in matchedList:
                tar_str = re.search('imgurl=(.*)&imgrefurl', m)
                try:
...
if __name__ == '__main__':
    ...
    if choice ==4:
        ...
        searchlist_filename = r'./imgsearch_list.txt'
